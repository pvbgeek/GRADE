#!/usr/bin/env python3
"""Run TutorMind inference with a Qwen3-14B LoRA adapter, thinking mode ON.

Purpose
-------
Load a task-specific Qwen3-14B LoRA adapter trained with thinking-mode prompts
and produce evaluator-compatible prediction CSVs for MI, ML, PG, Act, or MT,
with one retry pass for Unknowns. Runs with Qwen3 thinking mode ON.

Used by Experiment 2 — Thinking Mode (Runs 056-060).
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run TutorMind inference with a Qwen3-14B LoRA adapter (Think: ON)."
    )
...
def main() -> None:
    ...
    with track_emissions(f"cot-qwen14b-lora-infer-{args.task.lower()}"):
        ...
        print(
            ...
            f"using model {MODEL_NAME} (Think: ON) with adapter {adapter_path}."
        )
