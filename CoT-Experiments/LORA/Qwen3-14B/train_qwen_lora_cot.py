#!/usr/bin/env python3
"""Train a Qwen3-14B LoRA adapter for TutorMind with thinking mode ON.

Purpose
-------
Fine-tune a task-specific LoRA adapter for MI, ML, PG, Act, or MT using the
TutorMind chat-format training JSONL and the chain-of-thought prompts. Runs
with Qwen3 thinking mode ON.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train a Qwen3-14B LoRA adapter for a TutorMind task with thinking mode ON."
    )
...
def build_training_dataset(tokenizer: AutoTokenizer, train_path: Path, prompt: str) -> Dataset:
    ...
    for row_index, record in enumerate(rows):
        ...
        formatted_rows.append(
            {
                "text": apply_chat_template_think(
                    tokenizer,
                    messages,
...
def main() -> None:
    ...
    with track_emissions(f"cot-qwen14b-lora-train-{args.task.lower()}"):
        ...
        print(f"Saved Qwen3-14B LoRA adapter (Think: ON) for task {args.task} to {adapter_dir}")
