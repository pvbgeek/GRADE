#!/usr/bin/env python3
"""Run full-bf16 inference for one Qwen3-14B LoRA augmented run.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one full-bf16 Qwen3-14B LoRA inference run."
    )
...
def load_model_and_tokenizer(adapter_path: Path) -> tuple[AutoModelForCausalLM, AutoTokenizer]:
    ...
    print(f"Loading Qwen3-14B in full bfloat16 from {BASE_MODEL_PATH} ...")
...
def main() -> None:
    ...
    with track_emissions(f"dataaug-qwen14b-lora-infer-{task.lower()}"):
