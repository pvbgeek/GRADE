#!/usr/bin/env python3
"""Run TutorMind inference with a Llama-3.1-8B LoRA adapter.

Purpose
-------
Load a task-specific Llama-3.1-8B LoRA adapter and produce evaluator-compatible
prediction CSVs for MI, ML, PG, Act, or MT, with one retry pass for Unknowns.

Output
------
- Single-task CSVs with pred_label and raw_output
- MT CSVs with pred_mi, pred_ml, pred_pg, pred_act, and raw_output
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run TutorMind inference with a Llama-3.1-8B LoRA adapter."
    )
    ...
    parser.add_argument("--max-new-tokens", type=int, dlt=128)
...
def ensure_existing_dir(path_str: str, arg_name: str) -> Path:
    path = Path(path_str)
    if not path.is_dir():
...
def main() -> None:
    ...
    try:
        validate_args(args)
        adapter_path = ensure_existing_dir(args.adapter_path, "--adapter-path")
    ...
    with track_emissions(f"baseline-llama8b-lora-infer-{args.task.lower()}"):
        ...
        print(
            ...
            f"using model {MODEL_NAME} with adapter {adapter_path}."
        )
...
# -------
# Load a task-specific Llama-3.1-8B LoRA adapter and produce evaluator-compatible
# prediction CSVs for MI, ML, PG, Act, or MT.

# Input
# -----
# - A task-specific adapter directory created by ``train_lora.py``.
...
# Example
# -------
#     python '/WAVE/projects/CSEN-346-Sp26/Group3/TutorMind/Baseline-Experiments/LORA/Llama-3.1-8B/infer_lora.py' \\
#       --task MT \\
#       --adapter-path '/WAVE/projects/CSEN-346-Sp26/Group3/TutorMind/Baseline-Experiments/LORA/Llama-3.1-8B/adapters/MT' \\
#       --input-jsonl '/WAVE/projects/CSEN-346-Sp26/Group3/TutorMind/data/val/multitask_val.jsonl' \\
#       --predictions-out '/WAVE/projects/CSEN-346-Sp26/Group3/TutorMind/Baseline-Experiments/LORA/Llama-3.1-8B/predictions/run_020_mt_predictions.csv'
# """
...
# def ensure_existing_dir(path_str: str, arg_name: str) -> Path:
#     """Resolve a required directory path and ensure it exists."""
#     path = Path(path_str)
#     if not path.is_dir():
#         raise ValueError(f"{arg_name} does not exist or is not a directory: {path}")
#     return path.resolve()
...
# def select_prompt(prompts: dict, task: str) -> str:
#     """Select the configured LoRA inference prompt for the requested task."""
#     if task == "MT":
#         prompt = prompts["multitask_training"]["prompt"]
#     else:
#         prompt_key = SINGLE_TASK_PROMPT_KEYS[task]
...
# def load_model_and_tokenizer(adapter_path: Path) -> tuple[AutoModelForCausalLM, AutoTokenizer]:
#     """Load the hardcoded base model plus the requested LoRA adapter."""
#     tokenizer = load_tokenizer(BASE_MODEL_PATH)
#     if tokenizer.pad_token_id is None:
#         tokenizer.pad_token = tokenizer.eos_token
#     tokenizer.padding_side = "left"
...
# def main() -> None:
#     """Run LoRA inference and write evaluator-compatible predictions."""
#     parser = build_parser()
#     args = parser.parse_args()
