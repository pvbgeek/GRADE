#!/usr/bin/env python3
"""Run TutorMind inference with a Qwen3-14B LoRA adapter.

Purpose
-------
Load a task-specific Qwen3-14B LoRA adapter and produce evaluator-compatible
prediction CSVs for MI, ML, PG, Act, or MT, with one retry pass for Unknowns.
Runs with Qwen3 thinking mode OFF.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run TutorMind inference with a Qwen3-14B LoRA adapter."
    )
...
def strip_think_blocks(raw_output: str) -> str:
    # Qwen3 may still emit <think>...</think> wrappers even with thinking disabled;
    # scrub them so downstream label parsing sees only the final answer.
    if not raw_output:
        return raw_output
...
def render_prompt_texts(
    ...
    for offset, record in enumerate(records):
        ...
        rendered.append(
            apply_chat_template_no_think(tokenizer, messages, add_generation_prompt=True)
        )
    return rendered
...
def main() -> None:
    ...
    with track_emissions(f"scaling-qwen14b-lora-infer-{args.task.lower()}"):
            ...
            for offset, raw_output in enumerate(raw_outputs):
                ...
                if args.task == "MT":
                    ...
                    if UNKNOWN_LABEL in parsed.values():
                        ...
                        retry_prompt_text = apply_chat_template_no_think(
