...
#SBATCH -o /WAVE/projects/CSEN-346-Sp26/Group3/TutorMind/slurm-qwen3-%j.out
...
# ==========================================
# Experiment 1 (Scale) — Qwen3-14B runs 021-030
#   Zero-shot (Think: OFF) : runs 021-025
#   LoRA      (Think: OFF) : runs 026-030
# ==========================================
...
ZS_DIR="$ROOT/Scaling-Experiments/ZeroShot/Qwen3-14B"
LORA_DIR="$ROOT/Scaling-Experiments/LORA/Qwen3-14B"
EVAL_DIR="$ROOT/Scaling-Experiments/EVAL/Qwen3-14B"

ZS_SCRIPT="$ZS_DIR/zeroshot_qwen.py"
LORA_TRAIN_SCRIPT="$LORA_DIR/train_qwen_lora.py"
LORA_INFER_SCRIPT="$LORA_DIR/infer_qwen_lora.py"
EVAL_SCRIPT="$ROOT/evaluate_run.py"

MASTER_METRICS="$ROOT/master_metrics.csv"

RUN_STAMP="$(date +%Y%m%d_%H%M%S)"
LOG_DIR="$ROOT/Scaling-Experiments/logs/Qwen3-14B/$RUN_STAMP"
...
run_zero_shot() {
  ...
  local pred_csv="$ZS_OUT_DIR/run_${run_id}_qwen3_14b_zeroshot_${tag}_predictions.csv"
  local metrics_csv="$EVAL_DIR/run_${run_id}_qwen3_14b_zeroshot_${tag}_metrics.csv"

  local infer_log="$LOG_DIR/run_${run_id}_zeroshot_${tag}.log"
  local eval_log="$LOG_DIR/run_${run_id}_zeroshot_${tag}_eval.log"

  echo "===== RUN ${run_id} | Zero-shot | ${task} | Think: OFF ====="
...
run_lora() {
  ...
  local pred_csv="$LORA_OUT_DIR/run_${run_id}_qwen3_14b_lora_${tag}_predictions.csv"
  local metrics_csv="$EVAL_DIR/run_${run_id}_qwen3_14b_lora_${tag}_metrics.csv"
  ...
  echo "===== RUN ${run_id} | LoRA | ${task} | Think: OFF ====="
...
# ==========================================
# Execute Qwen3-14B scaling runs consecutively
# ==========================================

# Zero-shot runs 021-025 (Think: OFF)
run_zero_shot 021 MI   "$VAL_MI"  --val-mi  mi
run_zero_shot 022 ML   "$VAL_ML"  --val-ml  ml
run_zero_shot 023 PG   "$VAL_PG"  --val-pg  pg
run_zero_shot 024 Act  "$VAL_ACT" --val-act act
run_zero_shot 025 MT   "$VAL_MT"  --val-mt  mt

# LoRA runs 026-030 (Think: OFF)
run_lora 026 MI   "$TRAIN_MI"  "$VAL_MI"  --val-mi  mi
run_lora 027 ML   "$TRAIN_ML"  "$VAL_ML"  --val-ml  ml
run_lora 028 PG   "$TRAIN_PG"  "$VAL_PG"  --val-pg  pg
run_lora 029 Act  "$TRAIN_ACT" "$VAL_ACT" --val-act act
run_lora 030 MT   "$TRAIN_MT"  "$VAL_MT"  --val-mt  mt

echo "===== ALL QWEN3-14B SCALING RUNS COMPLETED ====="
echo "Logs:         $LOG_DIR"
