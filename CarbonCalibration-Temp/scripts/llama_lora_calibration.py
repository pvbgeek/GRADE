#!/usr/bin/env python3
"""CodeCarbon calibration for LLaMA-3.1-8B LoRA MI and MT.

Temporary calibration-only script.

Supports 6 LLaMA LoRA calibration runs:
- 006 = Original MI
- 010 = Original MT
- 061 = Gen MI
- 065 = Gen MT
...
# ---------------------------------------------------------------------
# Prompt and training-data helpers
# ---------------------------------------------------------------------
...
def build_project_base(run_id: str, task: str, aug: str) -> str:
    return f"calib_run{run_id}_llama31_8b_lora_{slugify_aug(aug)}_{task.lower()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CodeCarbon calibration for LLaMA-3.1-8B LoRA MI/MT across Original, Gen, and Gen+Verify."
    )
    ...
    parser.add_argument(
        ...
        help="006/010 original, 061/065 Gen, 086/090 Gen+Verify.",
    )
            ...
            for offset, raw_output in enumerate(raw_outputs):
                ...
                if task == "MT":
                    ...
                    if UNKNOWN_LABEL in parsed.values():
                        ...
                        retry_prompt_text = infer_tokenizer.apply_chat_template(
                            retry_messages,
                            tokenize=False,
                            add_generation_prompt=True,
                        )
    ...
    summary_row = {
        ...
        "notes": (
            "Retrospective LoRA calibration. Training prep scales by train examples; "
            "training fit/save scales by optimizer steps; inference generation scales by validation examples; "
            "model loads are counted once. Supports Original, Gen, and Gen+Verify LLaMA LoRA runs."
        ),
    }
    ...
    print()
    print("=" * 72)
    print("LoRA calibration complete")
    print("=" * 72)
