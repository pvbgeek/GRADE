#!/usr/bin/env python3
"""CodeCarbon calibration for Mistral-7B LoRA MI and MT.

Temporary calibration-only script.

Supports 6 Mistral LoRA calibration runs:
- 016 = Original MI
- 020 = Original MT
- 066 = Gen MI
- 070 = Gen MT
...
def load_infer_model_and_tokenizer(adapter_path: Path) -> tuple[AutoModelForCausalLM, AutoTokenizer]:
    ...
    if torch.cuda.is_available():
        print(f"GPU memory after base model load: {torch.cuda.memory_allocated() / 1024**3:.1f}GB")
...
def build_project_base(run_id: str, task: str, aug: str) -> str:
    return f"calib_run{run_id}_mistral7b_lora_{slugify_aug(aug)}_{task.lower()}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CodeCarbon calibration for Mistral-7B LoRA MI/MT across Original, Gen, and Gen+Verify."
    )
    ...
    parser.add_argument(
        ...
        help="016/020 original, 066/070 Gen, 091/095 Gen+Verify.",
    )
    ...
    # ------------------------------------------------------------------
    # 4. Inference model + adapter load
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 5. Inference generation on validation sample
    # ------------------------------------------------------------------
    ...
    summary_row = {
        ...
        "notes": (
            "Retrospective Mistral LoRA calibration. Training and inference use full bfloat16, "
            "matching the provided Mistral LoRA scripts. Training prep scales by train examples; "
            "training fit/save scales by optimizer steps; inference generation scales by validation examples; "
            "model loads are counted once."
        ),
    }
    ...
    print()
    print("=" * 72)
    print("Mistral LoRA calibration complete")
    print("=" * 72)
