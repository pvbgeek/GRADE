#!/usr/bin/env python3
"""CodeCarbon calibration for Qwen3-14B Think OFF LoRA MI and MT.

Temporary calibration-only script.

Supports 6 Qwen3-14B Think OFF LoRA calibration runs:
- 026 = Original MI
- 030 = Original MT
- 071 = Gen MI
- 075 = Gen MT
...
def build_project_base(run_id: str, task: str, aug: str) -> str:
    return f"calib_run{run_id}_qwen14b_thinkoff_lora_{slugify_aug(aug)}_{task.lower()}"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="CodeCarbon calibration for Qwen3-14B Think OFF LoRA MI/MT across Original, Gen, and Gen+Verify."
    )
    ...
    parser.add_argument(
        ...
        help="026/030 original, 071/075 Gen, 096/100 Gen+Verify.",
    )
    ...
    summary_row = {
        ...
        "notes": (
            "Retrospective Qwen3-14B Think OFF LoRA calibration. Training and inference use full bfloat16. "
            "Thinking is disabled via enable_thinking=False or /no_think fallback. Training prep scales by train examples; "
            "training fit/save scales by optimizer steps; inference generation scales by validation examples; model loads are counted once."
        ),
    }
    ...
    print()
    print("=" * 72)
    print("Qwen3-14B Think OFF LoRA calibration complete")
    print("=" * 72)
