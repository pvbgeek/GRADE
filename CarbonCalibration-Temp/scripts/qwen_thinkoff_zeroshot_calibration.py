#!/usr/bin/env python3
"""CodeCarbon calibration for Qwen3-14B Think OFF Zero-shot MI and MT.

Supported runs:
- 021 = Qwen3-14B Think OFF Zero-shot MI
- 025 = Qwen3-14B Think OFF Zero-shot MT
...
def build_project_base(run_id: str, task: str, load_in_4bit: bool) -> str:
    quant = "4bit" if load_in_4bit else "bf16"
    return f"calib_run{run_id}_qwen14b_thinkoff_zeroshot_{quant}_{task.lower()}"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="CodeCarbon calibration for Qwen3-14B Think OFF Zero-shot MI/MT."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["021", "025"],
        help="021 = Qwen Think OFF Zero-shot MI, 025 = Qwen Think OFF Zero-shot MT.",
    )
    ...
    run_id = run["run_id"]
    task = run["task"]
    project_base = build_project_base(run_id, task, args.load_in_4bit)
    ...
    summary_row = {
        ...
        "notes": (
            "Retrospective Qwen3-14B Think OFF zero-shot calibration. "
            "Thinking disabled via enable_thinking=False or /no_think fallback. "
            "Default is full bfloat16 unless --load-in-4bit is passed."
        ),
    }
    ...
    print()
    print("=" * 72)
    print("Qwen3-14B Think OFF zero-shot calibration complete")
    print("=" * 72)
