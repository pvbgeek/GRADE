#!/usr/bin/env python3
"""CodeCarbon calibration for Gemma3-12B Zero-shot MI and MT.

Supported runs:
- 031 = Gemma3-12B Zero-shot MI
...
def parse_multitask_output(text: str) -> dict[str, str]:
    field_map = {
...
def load_model():
    print(f"\nLoading {MODEL_NAME} from {MODEL_PATH} ...", flush=True)
...
def build_project_base(run_id: str, task: str) -> str:
    return f"calib_run{run_id}_gemma3_12b_zeroshot_{task.lower()}"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="CodeCarbon calibration for Gemma3-12B Zero-shot MI/MT."
    )
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["031", "035"],
        help="031 = Gemma3-12B Zero-shot MI, 035 = Gemma3-12B Zero-shot MT.",
    )
    ...
    summary_row = {
        ...
        "notes": (
            "Retrospective Gemma3-12B zero-shot calibration. "
            "Uses Gemma3ForConditionalGeneration + AutoProcessor, full bfloat16, "
            "device_map=auto, attn_implementation=eager. Load and inference tracked separately."
        ),
    }
    ...
    print()
    print("=" * 72)
    print("Gemma3-12B zero-shot calibration complete")
    print("=" * 72)
